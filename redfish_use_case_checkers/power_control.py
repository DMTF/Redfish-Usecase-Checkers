...
def power_test_reset_type(sut: SystemUnderTest, systems: list):
    ...
    for system in systems:
        ...
        if "Actions" not in system:
            sut.add_test_result(CAT_NAME, test_name, operation, "SKIP", "System '{}' does not support any actions.".format(system["Id"]))
            continue
        if "#ComputerSystem.Reset" not in system["Actions"]:
            sut.add_test_result(CAT_NAME, test_name, operation, "SKIP", "System '{}' does not support the 'Reset' action.".format(system["Id"]))
            continue
        ...
        try:
            ...
            sut.add_test_result(CAT_NAME, test_name, operation, "FAIL", "Failed to get the reset types supported for system '{}' ({}).".format(system["Id"], err))
...
def power_test_reset_operation(sut: SystemUnderTest, systems: list, reset_capabilities: dict):
    ...
    for reset_type in reset_types:
        ...
        # Reset each system
        for system in systems:
            operation = "Performing the reset action with the reset type '{}' for system '{}'".format(reset_type, system["Id"])
            logger.logger.info(operation)

            # Skip if the system does not support the reset action or shows reset capabilities
            if system["Id"] not in reset_capabilities:
                sut.add_test_result(CAT_NAME, test_name, operation, "SKIP", "System '{}' does not support the 'Reset' action or does not show supported reset types.".format(system["Id"]))
                continue

            # Skip if the system does not support the reset type
            if reset_type not in reset_capabilities[system["Id"]]:
                sut.add_test_result(CAT_NAME, test_name, operation, "SKIP", "System '{}' does not support the reset action with reset type '{}'.".format(system["Id"], reset_type))
                continue
        ...
        for system in systems:
            ...
            if reset_success[system["Id"]] == False:
                sut.add_test_result(CAT_NAME, test_name, operation, "SKIP", "System '{}' failed the reset action with reset type '{}'.".format(system["Id"], reset_type))
                continue

            # Skip if the system does not support reporting the power state
            if "PowerState" not in system:
                sut.add_test_result(CAT_NAME, test_name, operation, "SKIP", "System '{}' does not support the 'PowerState' property.".format(system["Id"]))
                continue
            ...
            try:
                ...
                logger.logger.info("Finished monitoring the power state for system '{}'; 'PowerState' contains '{}'".format(system["Id"], system_info.dict["PowerState"]))

                if system_info.dict["PowerState"] != expected_power_state:
                    sut.add_test_result(CAT_NAME, test_name, operation, "FAIL", "System '{}' did not transition to the '{}' power state after performing a reset of type '{}'.".format(system["Id"], expected_power_state, reset_type))
                else:
                    sut.add_test_result(CAT_NAME, test_name, operation, "PASS")
            except Exception as err:
                sut.add_test_result(CAT_NAME, test_name, operation, "FAIL", "Failed to monitor the power state for system '{}' ({}).".format(system["Id"], err))
